...
inline int reduceComponents(HyperGraph &hg, std::vector<int> &fixedLabels, double timeLimit, bool verbose = true) {
  ...
  int mipFailed = 0, satFailed = 0;

  int i;
  for(i = components.size() - 1; i >= 0 && elapsed() < timeLimit; i--) {
    if(components[i].size() > 14000) // Too much memory and likely takes too long
      break;
    HyperGraph component = hg.componentSubGraph(components[i]);

    Greedy greedy(component);
    greedy.solveGreedy();
    greedy.trim();
    // Greedy is optimal if it has at most 2 vertices
    if(greedy.getSolution().size() <= 2) {
        auto csol = greedy.getSolutionLabels();
      ...
      if(mipFailed < 2 && mip.solveExact(.5)) {
        auto csol = mip.getSolutionLabels();
        sol.insert(csol.begin(), csol.end());
        ret++;
      }
      else {
        mipFailed += (mipFailed < 2);
        MaxSAT maxsat(component);
        // double t = elapsed();
        double timeout = 5 + 8 * (i <= 2);
        double timeLeft = timeLimit - elapsed();
        timeout = std::min(timeLeft, timeout);
        if(timeout >= 4 && maxsat.solveExact(timeout, 1 + (i <= 3))) {
        ...
        if(satFailed >= 3) {
          i--; // Don't print this component, already printed
          break;
        }
      }
    }

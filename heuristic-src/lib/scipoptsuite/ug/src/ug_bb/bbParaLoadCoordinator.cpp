...
//        mergeInfoTail(0),
        ...
        aSolverTerminatedWithOptimality(false),
        pendingSolution(0)
{
...
//   }
   ...
   else
   {
      ...
      if( (!racingTermination) &&
            dynamic_cast<BbParaSolverPoolForMinimization *>(paraSolverPool) && // could be a specialized SolverPool for a specific problem solver
      ...
      )
{
   ...
   {
      if( ( i < static_cast<int>(paraSolverPool->getNSolvers()) + 1 && paraSolverPool->isSolverActive(i) && !paraSolverPool->isInterruptRequested(i) ) ||
            ( paraRacingSolverPool && i < static_cast<int>(paraRacingSolverPool->getNSolvers()) + 1 && paraRacingSolverPool->isSolverActive(i) ) )
      {
      ...
      if( i < static_cast<int>(paraSolverPool->getNSolvers()) + 1 && !paraSolverPool->isTerminateRequested(i) )
      {
         PARA_COMM_CALL(
      ...
      )
{
   ...
   if( updateSolution(sol) ) 
   {
      ...
      char solutionFileNameTemp[MaxStrLen];
      char solutionFileName[MaxStrLen];
      if( paraParams->getBoolParamValue(Checkpoint) && paraComm->getRank() == 0  )
      {
         snprintf(solutionFileNameTemp, MaxStrLen, "%s%s_after_checkpointing_solution_t.gz",
         ...
         snprintf(solutionFileName, MaxStrLen, "%s%s_after_checkpointing_solution.gz",
               paraParams->getStringParamValue(CheckpointFilePath),
               paraInitiator->getParaInstance()->getProbName());
...
//      assert( solverState->getNNodesSolved() >= dynamic_cast<BbParaSolverPoolForMinimization *>(paraSolverPool)->getNumOfNodesSolved(source) );
   ...
   // std::cout << "givenGapIsReached = " << givenGapIsReached << std::endl;
   if( !givenGapIsReached )
   {
      // std::cout << "absgap = " <<
      ...
      {
         // std::cout << "current dual = "  << bbParaInitiator->convertToExternalValue(solverState->getSolverLocalBestDualBoundValue()) <<std::endl;
         // std::cout << "current gap = " << bbParaInitiator->getGap(solverState->getSolverLocalBestDualBoundValue()) <<std::endl;
      ...
      else
      {
         ...
         {
            if ( paraSolverPool->isSolverActive(source) && !paraSolverPool->isInterruptRequested(source) )
            {
               PARA_COMM_CALL(
      ...
      )
{
   ...
   if( logSolvingStatusFlag )
   {
      ...
      *osLogSolvingStatus << ", " << dynamic_cast<BbParaNode *>(paraSolverPool->getCurrentTask(source))->toSimpleString();
...
// std::cout << "sendInterrruptRequest 3" << std::endl;
      ...
      if( (!paraRacingSolverPool) || (!paraRacingSolverPool->isSolverActive(source) ) )  // RacingSolverPool is inactivated below
      {
         ...
         if( paraNodePoolToRestart )
         {
            ...
            {
               if( bbParaInitiator->getAbsgap(solvingNode->getDualBoundValue()) < bbParaInitiator->getAbsgapValue() ||
                     bbParaInitiator->getGap(solvingNode->getDualBoundValue()) < bbParaInitiator->getGapValue() )
               {
                  delete solvingNode;
               }
               else
               ...
               && !racingWinnerParams )
   {
      ...
      if( !racingTermination && paraRacingSolverPool )
      {
         ...
         *osTabularSolvingStatus << std::setw(10) << std::right << paraRacingSolverPool->getNumActiveSolvers();
         ...
         {
            if( bbParaInitiator->getGlobalBestIncumbentSolution() && EPSEQ( lcts.globalBestDualBoundValue, bbParaInitiator->getGlobalBestIncumbentSolution()->getObjectiveFunctionValue(), paraInitiator->getEpsilon() ) )
            {
               *osTabularSolvingStatus << std::setw(17) << std::right << std::setprecision(4) <<
   ...
   for(;;)
   {
      ...
      if( !paraRacingSolverPool && paraSolverPool->getNumActiveSolvers() == 0 )
      {
         if( paraParams->getRealParamValue(TimeLimit) > 0.0 )
         {
             ...
             if( paraSolverPool->getNumActiveSolvers() == 0
                 && ( paraNodePool->isEmpty() || aSolverTerminatedWithOptimality )
      ...
      )
{
   ...
   char nodesFileName[MaxStrLen];
   snprintf(nodesFileName, MaxStrLen, "%s%s_%s_nodes_LC%d.gz",
         paraParams->getStringParamValue(CheckpointFilePath),
   ...
   {
      snprintf(solutionFileName, MaxStrLen, "%s%s_%s_solution.gz",
            paraParams->getStringParamValue(CheckpointFilePath),
   ...
   char solverStatisticsFileName[MaxStrLen];
   snprintf(solverStatisticsFileName, MaxStrLen, "%s%s_%s_solverStatistics_LC%d.gz",
         paraParams->getStringParamValue(CheckpointFilePath),
         paraInitiator->getParaInstance()->getProbName(), newCheckpointTimeStr, paraComm->getRank());
   ...
   char loadCoordinatorStatisticsFileName[MaxStrLen];
   snprintf(loadCoordinatorStatisticsFileName, MaxStrLen, "%s%s_%s_loadCoordinatorStatistics_LC%d.gz",
         paraParams->getStringParamValue(CheckpointFilePath),
         paraInitiator->getParaInstance()->getProbName(), newCheckpointTimeStr, paraComm->getRank());
   ...
   if( lastCheckpointTimeStr[0] == ' ' )
   {
      ...
      if( racingWinnerParams )
      {
         ...
         char racingWinnerParamsName[MaxStrLen];
         snprintf(racingWinnerParamsName, MaxStrLen, "%s%s_racing_winner_params.gz",
               paraParams->getStringParamValue(CheckpointFilePath),
               paraInitiator->getParaInstance()->getProbName());
   ...
   char afterCheckpointingSolutionFileName[MaxStrLen];
   snprintf(afterCheckpointingSolutionFileName, MaxStrLen, "%s%s_after_checkpointing_solution.gz",
         paraParams->getStringParamValue(CheckpointFilePath),
         paraInitiator->getParaInstance()->getProbName());
      ...
      )
{
   ...
   char loadCoordinatorStatisticsFileName[MaxStrLen];
   snprintf(loadCoordinatorStatisticsFileName, MaxStrLen, "%s_loadCoordinatorStatistics_LC0.gz", paraInitiator->getPrefixWarm());
   ...
   char solverStatisticsFileName[MaxStrLen];
   snprintf(solverStatisticsFileName, MaxStrLen, "%s_solverStatistics_LC0.gz", paraInitiator->getPrefixWarm());
   ...
   char previousStatisticsFileName[MaxStrLen];
   snprintf(previousStatisticsFileName, MaxStrLen, "%s_statistics_w%05lld_LC0",
         paraInitiator->getPrefixWarm(),
         prevLcts->nWarmStart);
      ...
      )
{
   ...
   paraRacingSolverPool = new BbParaRacingSolverPool(
         nRacingSolvers,   // the number of racing solvers
         1,                // paraSolver origin rank
   ...
   for(;;)
   {
      ...
      case RampUpPhase:
      {
         ...
         if( racingTermination )
         {
            ...
            {
               if( source < static_cast<int>(paraSolverPool->getNSolvers()) + 1 && !paraSolverPool->isInterruptRequested(source) )
               {
                  sendInterruptRequest();
...
// std::cout << "TagTerminateRequest 5" << std::endl;
         ...
         if( paraRacingSolverPool && paraRacingSolverPool->getNumActiveSolvers() > 0 )
         {
            for( int i = 1; i < static_cast<int>(paraRacingSolverPool->getNSolvers()) + 1; i++ )
            {
               if( paraRacingSolverPool->isSolverActive(i) && !paraSolverPool->isInterruptRequested(i) )
               {
                  PARA_COMM_CALL(

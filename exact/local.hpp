...
protected:
  ...
  template<bool useQueue = true>
  std::vector<int> oldrandomRemovals(int limit) {
    limit = std::min(limit, hg->countVertices());

    //random on left
    // std::vector<int> vertexPool; // Use random pool
    std::list<int> vertexPool; // Use queue

    std::list<int> edgeQueue;
    std::unordered_set<int> pooledVertices, queuedEdges;
    std::vector<int> removed;

    int rv = randomVertex(rgen);
    vertexPool.push_back(rv);
    pooledVertices.insert(rv);

    while((int)unhit.size() < limit || (int)removed.size() < limit/2) {
      while(vertexPool.empty()) { // Fill vertex pool
        if(edgeQueue.empty())
          return removed;  // Failed to reach limit. Small connected component!
          int e = edgeQueue.front();
        edgeQueue.pop_front();
        for(int v : randomPermutation(hg->edges[e])) {
          if(!pooledVertices.contains(v)) {
            pooledVertices.insert(v);
            vertexPool.push_back(v);
          }
        }
      }

      // Pick vertex from pool
      // int v = extractRandomElement(vertexPool);
      int v = vertexPool.front();
      vertexPool.pop_front();

      // Add edges to queue
      for(int e : randomPermutation(hg->edgesContaining[v])) {
        if(!queuedEdges.contains(e)) {
          queuedEdges.insert(e);

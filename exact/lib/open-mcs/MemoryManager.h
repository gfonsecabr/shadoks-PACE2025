...
static void* MallocWithCheck(size_t x)
{
    ...
    #else
    void* retvalue = malloc(std::max(x,1u));
    #endif
...
static void* CallocWithCheck(size_t x, size_t y)
{
    ...
    #else
    void* retvalue = calloc(std::max(x,1u),std::max(y,1u)); 
    #endif
...
#define Free(x) free(x)
    ...
    #define Malloc(x) malloc(/*std::max(x,1)*/x)
    #define Calloc(x,y) calloc(/*std::max(x,1)*/x,/*std::max(y,1)*/y)
    #define Free(x) free(x)

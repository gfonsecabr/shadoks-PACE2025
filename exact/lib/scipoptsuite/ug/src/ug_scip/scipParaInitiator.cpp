      ...
      )
{
                                 ...
                                 /** the second argument is problem file name */
   {
      ...
      else if( strcmp(argv[i], "-w") == 0)
      {
         ...
         {
            prefixWarm = argv[i];
            char nodesFileName[SCIP_MAXSTRLEN];
            (void) SCIPsnprintf(nodesFileName, SCIP_MAXSTRLEN, "%s_nodes_LC0.gz", prefixWarm);
...
#endif
      else if ( strcmp(argv[i], "-omit1" ) == 0 )   /// to omit this parameter
      {
      }
      else if ( strcmp(argv[i], "-omit2" ) == 0 )   /// to omit this parameter and the following value
      {
         i++;
      }
      else
   ...
   if( probname != NULL )
   {
      ...
      if( settingsName != NULL )
      {
         ...
         if( getAbsgapValue(paramScip) > MINEPSILON || getGapValue(paramScip) > MINEPSILON )
         {
            if( paraParams->getIntParamValue(RampUpPhaseProcess) == 2 )
            {
               paraParams->setIntParamValue(RampUpPhaseProcess, 1);
               std::cout << "** RampUpPhaseProcess is switched to 1, since gap for solvers is specified" << std::endl;
            }
         }
      }
      else
      {
         ...
         if( getAbsgapValue(scip) > MINEPSILON || getGapValue(scip) > MINEPSILON )
         {
            if( paraParams->getIntParamValue(RampUpPhaseProcess) == 2 )
            {
               paraParams->setIntParamValue(RampUpPhaseProcess, 1);
      ...
      if( settingsNameRoot != NULL )
      {
         ...
         if( getAbsgapValue(paramScip) > MINEPSILON || getGapValue(paramScip) > MINEPSILON )
         {
            ...
            {
               paraParams->setIntParamValue(RampUpPhaseProcess, 1);
               std::cout << "** RampUpPhaseProcess is switched to 1, since gap for root node is specified" << std::endl;
            }
         }
      }
      else
      ...
      if( settingsNameLC != NULL )
      {
         // SCIP_CALL( SCIPresetParams(scip) );
         SCIP_CALL_ABORT( SCIPreadParams(scip, settingsNameLC) );
         std::cout << "** applied absolute gap = " << getAbsgapValue() << std::endl;
         std::cout << "** applied relative gap = " << getGapValue() << std::endl;
         if( getAbsgapValue() > MINEPSILON || getGapValue() > MINEPSILON )
         {
            if( paraParams->getIntParamValue(RampUpPhaseProcess) == 2 )
            {
               paraParams->setIntParamValue(RampUpPhaseProcess, 1);
               std::cout << "** RampUpPhaseProcess is switched to 1, since gap is specified" << std::endl;
            }
         }
      }
...
// #endif
      ...
      if( paraParams->getIntParamValue(UG::InstanceTransferMethod) == 1 )
      {
         if( onlyLinearConss )
         {
      ...
      )
{
   ...
   char initSolFileName[SCIP_MAXSTRLEN];

   if( isolname )
   {
      (void) SCIPsnprintf(initSolFileName, SCIP_MAXSTRLEN, "%s.%d", isolname, nRestartedRacing);
      ...
      {
         (void) SCIPsnprintf(initSolFileName, SCIP_MAXSTRLEN, "%s", isolname);
   ...
   else
   {
      if( !generatedIsolname )
      {
         (void) SCIPsnprintf(initSolFileName, SCIP_MAXSTRLEN, "i_%s.sol", instance->getProbName());
         generatedIsolname = new char[strlen(initSolFileName)+1];
         (void) strcpy(generatedIsolname, initSolFileName);
      }
   }
...
{
   char tempSolutionFileName[SCIP_MAXSTRLEN];
   (void) SCIPsnprintf(tempSolutionFileName, SCIP_MAXSTRLEN, "%s_solution.gz", prefixWarm);

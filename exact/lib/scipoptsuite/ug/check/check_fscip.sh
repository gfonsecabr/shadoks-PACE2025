...
HARDTIMELIMIT=`expr \`expr $TIMELIMIT \* $THREADS\` + $TIMELIMIT + 600`
...
do
    ...
    then
        ...
        then
            ...
            echo -----------------------------
            date +"@03 %s"
            echo "../$BINNAME $TMPFILE $i -sth $THREADS"
            bash -c "ulimit -t $HARDTIMELIMIT s;../$BINNAME $TMPFILE $i -sth $THREADS -q" 2>>$ERRFILE
